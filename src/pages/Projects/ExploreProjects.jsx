            ...
            {sortedProjects.map((project, index) => {
              ...
              return (
                <TiltCard 
                  key={project.id} 
                  delay={index * 100}
                  className={`glass-card bg-surface p-6 rounded-2xl shadow-sm border transition-shadow flex flex-col h-full relative group ${project.status === 'deactivated' ? 'border-red-200 bg-red-50 hover:shadow-md' : 'border-gray-100 hover:shadow-md'}`}
                >
                  <div className="absolute top-4 right-4 flex flex-col gap-2 items-end">
                    {canSaveFavorites && (
                      <button 
                        onClick={(e) => { e.preventDefault(); toggleFavorite(currentUser.id, project.id, 'project'); }}
                        className="animate-pop p-2 bg-white rounded-full border border-gray-100 hover:bg-red-50 text-gray-400 hover:text-red-500 shadow-sm transition-all hover:scale-110 z-20 relative"
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center relative z-10">
                      <Folder className="w-5 h-5" />
                    </div>
                    <div className="flex items-center text-yellow-500 text-sm font-bold mt-2 mr-12 relative z-10">
                  ...
                  <p className="text-sm text-gray-500 mb-2 line-clamp-1 relative z-10">{getCourseName(project.courseId)}</p>
                  
                  <div className="flex flex-wrap gap-2 mb-4 flex-1 relative z-10">
                  ...
                  <div className={`pt-4 border-t flex items-center justify-between mt-auto relative z-10 ${project.status === 'deactivated' ? 'border-red-100' : 'border-gray-100'}`}>
