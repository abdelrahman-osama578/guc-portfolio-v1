        ...
        {sortedProjects.map((project, index) => {
          ...
          return (
            <TiltCard
              key={project.id}
              delay={index * 100}
              className={`glass-card bg-surface rounded-2xl shadow-sm border flex flex-col h-full relative group overflow-hidden ${project.status === 'deactivated' ? 'border-red-200 bg-red-50' : 'border-gray-100 hover:shadow-md'}`}
            >
              {/* FIXED: Padded Flex layout is now safely inside! */}
                  ...
                  <div className="flex items-center gap-2">
                    {canSaveFavorites && (
                      <button
                        ...
                        className="animate-pop p-1.5 bg-white rounded-full border border-gray-100 hover:bg-red-50 text-gray-400 hover:text-red-500 shadow-sm transition-all hover:scale-110 z-20"
